...
print_ax:
  ...
  .print_buf:
    ...
    .print_char_loop:
      ...
      push cx  ; I'll explain this push/pop stuff in a moment
        push dx
          push ax
            xor cx, cx
            mov dx, 0xffff
...
stack_example:
  ...
  push ax  ; Pushes the value onto the stack, saving it
    call print_ax  ; Oh yeah BTW we DO have functions! And this is how you call them... It saves `ip`, the instruction pointer that I mentioned earlier, such that it may be recovered later and we can pick up from right after this call (via the use of a RET opcode)
...
daps:
  ...
  ; NUM_SECTORS comes from the environment, see the makefile (+2 in stage three to account for user data)
  %ifdef STAGE_3
   .num_sectors:      dw NUM_SECTORS + 2
  %else
